...
document.addEventListener('DOMContentLoaded', async function () {
    try {
        const res = await fetch('api/get_products.php'); // adjust path if different
        products = await res.json();

        // Safer parsing of features/specs
        products.forEach(p => {
            try {
                p.stock = parseInt(p.stock); // ✅ convert stock to number

                if (typeof p.features === "string" && p.features.trim().startsWith("[")) {
                ...
                console.warn("Invalid product JSON fields", p, e);
                p.features = [];
                p.specifications = {};
            }
        });
        ...
        showNotification('Failed to load products from the store.', 'error');
    }
});
            ...
            filteredProducts.forEach(product => {
                ...
                productCard.innerHTML = `
                    <div class="product-image">
                        <img src="${product.image_paths}" alt="${product.name}" class="product-thumb" style="max-width: 100%; height: auto;">
                    </div>
