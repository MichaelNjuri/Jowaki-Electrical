<?php
session_start();
require_once 'db_connection.php';
require_once 'check_auth.php';

header('Content-Type: application/json');

// Check if user is admin
if (!isAdmin()) {
    http_response_code(403);
    echo json_encode(['success' => false, 'message' => 'Access denied. Admin privileges required.']);
    exit;
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $conn = getConnection();
    if (!$conn) {
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Database connection failed']);
        exit;
    }

    try {
        $data = json_decode(file_get_contents('php://input'), true);
        
        if (!$data) {
            $data = $_POST;
        }

        $admin_id = $data['admin_id'] ?? null;
        $username = $data['username'] ?? '';
        $email = $data['email'] ?? '';
        $first_name = $data['first_name'] ?? '';
        $last_name = $data['last_name'] ?? '';

        // Validate required fields
        if (!$admin_id) {
            echo json_encode(['success' => false, 'message' => 'Admin ID is required']);
            exit;
        }

        if (empty($username) || empty($email) || empty($first_name) || empty($last_name)) {
            echo json_encode(['success' => false, 'message' => 'All fields are required']);
            exit;
        }

        // Validate email format
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            echo json_encode(['success' => false, 'message' => 'Invalid email format']);
            exit;
        }

        // Check if username already exists (excluding current admin)
        $check_username = $conn->prepare("SELECT id FROM admin_users WHERE username = ? AND id != ?");
        $check_username->bind_param("si", $username, $admin_id);
        $check_username->execute();
        if ($check_username->get_result()->num_rows > 0) {
            echo json_encode(['success' => false, 'message' => 'Username already exists']);
            exit;
        }
        $check_username->close();

        // Check if email already exists (excluding current admin)
        $check_email = $conn->prepare("SELECT id FROM admin_users WHERE email = ? AND id != ?");
        $check_email->bind_param("si", $email, $admin_id);
        $check_email->execute();
        if ($check_email->get_result()->num_rows > 0) {
            echo json_encode(['success' => false, 'message' => 'Email already exists']);
            exit;
        }
        $check_email->close();

        // Update admin profile
        $stmt = $conn->prepare("UPDATE admin_users SET username = ?, email = ?, first_name = ?, last_name = ? WHERE id = ?");
        $stmt->bind_param("ssssi", $username, $email, $first_name, $last_name, $admin_id);
        
        if ($stmt->execute()) {
            // Log activity
            logAdminActivity('Update Profile', "Updated profile for admin ID: $admin_id");
            
            // Get updated admin data
            $get_admin = $conn->prepare("SELECT id, username, email, first_name, last_name, role_id FROM admin_users WHERE id = ?");
            $get_admin->bind_param("i", $admin_id);
            $get_admin->execute();
            $admin_data = $get_admin->get_result()->fetch_assoc();
            
            $stmt->close();
            $get_admin->close();
            $conn->close();

            echo json_encode([
                'success' => true,
                'message' => 'Profile updated successfully',
                'admin' => [
                    'id' => $admin_data['id'],
                    'username' => $admin_data['username'],
                    'email' => $admin_data['email'],
                    'first_name' => $admin_data['first_name'],
                    'last_name' => $admin_data['last_name'],
                    'full_name' => $admin_data['first_name'] . ' ' . $admin_data['last_name'],
                    'role_id' => $admin_data['role_id']
                ]
            ]);
        } else {
            echo json_encode(['success' => false, 'message' => 'Failed to update profile']);
        }

    } catch (Exception $e) {
        http_response_code(500);
        echo json_encode([
            'success' => false,
            'message' => 'Error updating profile: ' . $e->getMessage()
        ]);
    }
} else {
    http_response_code(405);
    echo json_encode(['success' => false, 'message' => 'Method not allowed']);
}
?>

