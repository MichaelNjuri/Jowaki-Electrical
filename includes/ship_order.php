...
try {
    ...
    $shippingNotes = isset($input['shippingNotes']) ? trim($input['shippingNotes']) : '';
    $trackingNumber = isset($input['trackingNumber']) ? trim($input['trackingNumber']) : '';
    ...
    if (!in_array($order['status'], ['confirmed', 'processing'])) {
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => 'Order cannot be shipped. Current status: ' . $order['status']]);
        exit;
    }
    
    // Update order status to shipped
    $updateStmt = $conn->prepare("UPDATE orders SET status = 'shipped', updated_at = NOW() WHERE id = ?");
