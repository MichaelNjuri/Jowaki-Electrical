...
try {
    ...
    // Process customer information from JSON or user table
    $customer_info = [];
    
    // First try to get from customer_info JSON field
    if (!empty($order['customer_info'])) {
        $customer_info = json_decode($order['customer_info'], true) ?: [];
    }
    
    // If no customer_info JSON, fall back to user table data
    ...
    if (empty($customer_info['name']) && !empty($customer_info['firstName']) && !empty($customer_info['lastName'])) {
        $customer_info['name'] = $customer_info['firstName'] . ' ' . $customer_info['lastName'];
    }

    // Process cart items from JSON if order_items table is empty
    if (empty($items) && !empty($order['cart'])) {
        ...
        foreach ($cartItems as $item) {
            $items[] = [
                ...
                'total' => floatval(($item['quantity'] ?? 0) * ($item['price'] ?? 0))
            ];
        }
    }
